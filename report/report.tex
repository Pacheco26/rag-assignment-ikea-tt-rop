...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/overall_strategy_summary.png}}
...
We implement extractive-abstractive summarization using a map-reduce approach. For retrieved chunks, the summarizer first generates a per-chunk summary, then combines these into a coherent synthesis focused on the user's query. For full-document summarization, it summarizes each section independently (map step) and then merges the section summaries into a document-level summary (reduce step). The LLM (via Groq) handles the abstractive generation.
...
\begin{itemize}
    ...
    \item \textit{Source Coverage}: fraction of gold-standard documents retrieved
\end{itemize}
...
\toprule
\textbf{Strategy} & \textbf{MRR} & \textbf{nDCG} & \textbf{Rec@5} & \textbf{Corr.} & \textbf{Faith.} & \textbf{Cit. F1} \\
\midrule
Fixed-Size & \textbf{0.811} & \textbf{0.663} & \textbf{0.670} & \textbf{0.642} & 0.773 & 0.506 \\
Semantic & 0.781 & 0.649 & 0.652 & 0.620 & \textbf{0.802} & \textbf{0.520} \\
...
\textbf{Fixed-size} achieves the best retrieval (MRR\,=\,0.811) and correctness (0.642). Its consistent chunk sizes produce reliable retrieval behavior.
...
\textbf{Hierarchical} underperforms on all retrieval metrics. We discuss why in Section~VIII.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/retrieval_metrics_comparison.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/answer_quality_comparison.png}}
